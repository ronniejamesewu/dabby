...
| Rig 3 (sapphire insert — Dr. Dabber Sapphire Plus v2) | First run: ultra-clean swab (essentially white/cream, cleanest in OC history), very strong delayed effect at 420°F, harshness 11s into the 420°F hold. Consistent with sapphire running 10–20°F hotter in effective temperature than quartz at the same setpoint. | OC Run 8 (May 22, 2026) — one data point | Low — single run | Empirical calibration from scratch. Do not scale from quartz curves. First data point is consistent with the 10–20°F lower setpoint reddit consensus but one run does not confirm it. Fresh data will accumulate on subsequent runs. |
