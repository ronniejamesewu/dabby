...
| Rig 3 (sapphire insert — Dr. Dabber Sapphire Plus v2) | Run 8 (420°F): ultra-clean swab, very strong delayed effect, harshness 11s into the hold. Run 9 (415°F, small load): clean first draw (25s, no harshness), wispy vapor, trace harshness only as load ran out. Directionally cleaner at 415°F but small load confounds the result. Wispy vapor on first draw may indicate 415°F is slightly below the sapphire's vapor production sweet spot. | OC Runs 8–9 (May 22, 2026) — two data points, one endpoint each | Low — two runs, load confound on Run 9 | Empirical calibration from scratch. Do not scale from quartz curves. 420°F produced harshness; 415°F directionally cleaner but unconfirmed. Run 10 repeats 415°F with a fuller load to isolate the endpoint variable. |
