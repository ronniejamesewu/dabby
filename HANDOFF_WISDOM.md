...
| Bitter citrus / citrus rind note recurs across strains | MB9ZST R1 (tangerine/bitter citrus), MBD R4 (citrus rind), OC R12 (bitter citrus — user: "increasingly familiar") | Low — three strains, different producers, different genetics; note recurs across multiple sessions | Sessions 39, 47, 70 | Origin unknown. Limonene is inferred in all three strains' terpene profiles — could be limonene expression at these temperature ranges, or an unrelated coincidence. Not worth weighting in per-run analysis until a mechanism or genetic correlation is established. Watch for recurrence. |
...
| Rig 4 (sapphire insert + 5mm synthetic ruby pearl) | Ruby is corundum (same material as the sapphire insert, Al₂O₃) — ~2x volumetric heat capacity and ~20x thermal conductivity over quartz. Despite the smaller diameter, the 5mm ruby carries comparable absolute thermal mass to the 6mm quartz pearl (higher density and heat capacity offset the volume reduction). Full corundum pathway (insert and pearl both Al₂O₃) likely runs at higher effective temperature than sapphire + quartz pearl at the same setpoint. FW106 R1 (baseline, 416°F): very clean swab, strong terpene expression, terpene-load cough without harshness, mild harshness at 19s into 416°F hold. | FW106 R1 (May 25, 2026 — very clean swab, terpene-load cough without harshness, mild harshness at 19s into 416°F hold, medium intensity) | Low — single run, new strain; no cross-rig comparison yet | Whether corundum pearl pushes harshness earlier (denser vapor) or later (material done sooner) is unresolved — the two competing harshness mechanisms predict opposite outcomes. Cross-rig comparison on the same strain (e.g. OC on Rig 4 vs. Rig 3) needed to isolate pearl effect from strain character. |
