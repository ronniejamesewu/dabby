...
COMPLETED_RUNS = [
    ...
    CompletedRun(strain="Watermellos", run_date=date(2026, 6, 10), sessions_prior_today=0, utc_logged_at=datetime(2026, 6, 10, 23, 4, tzinfo=timezone.utc), equipment=RIG_5, waypoints=FW106_FASTER_420,
        ...
        analysis="Run 12 matched Run 11's structure (2x2) and returned the same dark gold swab. Two consecutive dark gold runs now, breaking the streak of beige across Runs 2–10 on Rig 5. The second-cycle-heat isolation still hasn't happened — this run also went two cycles, so the planned single-cycle test didn't land. What's confirmed: dark gold is reproducible across two consecutive 2x2 sessions. Draw-count pattern held — harshness crept in at the tail of draw 2 and escalated through draws 3–4, consistent with the established ceiling. Mildew on draw 1 is a recurring WM flavor character (Runs 5, 6, 7, 8, 10). Watermelon note on draw 2 is the strain's namesake showing up clearly. Heavy fast-onset effect consistent with WM's history on Rig 5.",
    ),
    CompletedRun(strain="Watermellos", run_date=date(2026, 6, 10), sessions_prior_today=1, utc_logged_at=datetime(2026, 6, 11, 3, 25, tzinfo=timezone.utc), equipment=RIG_5, waypoints=FW106_FASTER_420,
        ...
        analysis="Three consecutive dark gold swabs on 2x2 sessions (Runs 11–13). Single-cycle isolation still hasn't happened, but the streak is now three runs — dark gold is reproducible, not a two-run anomaly. Harshness on the 4th draw with the second cycle cut short fits the established pattern: second cycle is scraping the bottom on a hot insert. 'Not very tasty' is a new note — Runs 10–12 all had flavor present (mildew, watermelon). Diminished flavor alongside persistent dark gold is directionally consistent with the quality-shift read from Run 11. Very hard effect.",
    ),
