...
STRAIN_STATUS = [
    StrainStatus(name="WW Z", profile_anchor="#wwz-profile", next_text="Repeat same curve without push-button extension (stay at 50s max) for clean swab confirmation", accent=None, slug="wwz",
        info=WWZ_INFO,
        ...
        next_dab_notes="Runs 3–4 (same curve): harshness both at 414–420°F on ramp, never at hold. Run 3 very clean swab, Run 4 amber (push-button extension, not curve). Load ballpark. Effect consistent: creative, sharp, stoned.",
        next_ai_analysis="Two runs at this curve (380→410@15s→430@30s, hold to ~50s). Harshness appears mid-ramp at 414–420°F on display on both — not at the endpoint hold. The range across runs is consistent with load and attention variability, not a fixed temperature signal. Swabs have been clean in both (Run 4's amber is push-button confound, not the curve). Effect character is consistent and good. Run 5: repeat same curve but skip the push-button extension — stay at 50s max. That gets a clean swab comparison without the overshoot confound. Don't touch the endpoint yet — no unconfounded floor signal.",
