...
COMPLETED_RUNS = [
    ...
    CompletedRun(strain="Watermellos", run_date=date(2026, 6, 9), sessions_prior_today=0, utc_logged_at=datetime(2026, 6, 9, 23, 20, tzinfo=timezone.utc), equipment=RIG_5, waypoints=FW106_FASTER_420,
        duration_seconds=40, endpoint_note='<strong>Endpoint:</strong> 420°F — 8s fast ramp; up 4°F from prior runs',
        swab="Beige.",
    ...
    CompletedRun(strain="Watermellos", run_date=date(2026, 6, 9), sessions_prior_today=1, utc_logged_at=datetime(2026, 6, 10, 2, 3, tzinfo=timezone.utc), equipment=RIG_5, waypoints=FW106_FASTER_420,
        duration_seconds=40, endpoint_note='<strong>Endpoint:</strong> 420°F — 8s fast ramp; same as Run 10',
        swab="Dark golden.",
    ...
    CompletedRun(strain="Watermellos", run_date=date(2026, 6, 10), sessions_prior_today=0, utc_logged_at=datetime(2026, 6, 10, 23, 4, tzinfo=timezone.utc), equipment=RIG_5, waypoints=FW106_FASTER_420,
        duration_seconds=40, endpoint_note='<strong>Endpoint:</strong> 420°F — 8s fast ramp; same as Run 11',
        swab="Dark gold.",
    ...
    CompletedRun(strain="Watermellos", run_date=date(2026, 6, 10), sessions_prior_today=1, utc_logged_at=datetime(2026, 6, 11, 3, 25, tzinfo=timezone.utc), equipment=RIG_5, waypoints=FW106_FASTER_420,
        duration_seconds=40, endpoint_note='<strong>Endpoint:</strong> 420°F — 8s fast ramp; same as Runs 11–12',
        swab="Dark gold.",
    ...
    CompletedRun(strain="Watermellos", run_date=date(2026, 6, 11), sessions_prior_today=1, utc_logged_at=datetime(2026, 6, 12, 3, 11, tzinfo=timezone.utc), equipment=RIG_5, waypoints=WM_RUN15_NEXT,
        duration_seconds=60, endpoint_note='<strong>Endpoint:</strong> 420°F — 8s ramp; 60s hold (extended from 40s), single cycle',
        swab="Beige.",
    ...
    CompletedRun(strain="Fire Water #106", run_date=date(2026, 6, 7), sessions_prior_today=0, utc_logged_at=datetime(2026, 6, 8, 1, 50, tzinfo=timezone.utc), equipment=RIG_5, waypoints=FW106_FASTER_420,
        duration_seconds=40, endpoint_note='<strong>Endpoint:</strong> 420°F — 8s ramp; same as Run 13',
    ...
    CompletedRun(strain="Fire Water #106", run_date=date(2026, 6, 8), sessions_prior_today=0, utc_logged_at=datetime(2026, 6, 9, 1, 0, tzinfo=timezone.utc), equipment=RIG_5, waypoints=FW106_FASTER_420,
        duration_seconds=40, endpoint_note='<strong>Endpoint:</strong> 420°F — 8s ramp; same as Runs 13–14 &nbsp;|&nbsp; <strong>Note:</strong> Second heating cycle added',
    ...
    CompletedRun(strain="Fire Water #106", run_date=date(2026, 6, 12), sessions_prior_today=0, utc_logged_at=datetime(2026, 6, 12, 23, 42, tzinfo=timezone.utc), equipment=RIG_4, waypoints=FW106_FASTER_420_60S,
        duration_seconds=60, endpoint_note='<strong>Endpoint:</strong> 420°F — 8s ramp; 60s hold &nbsp;|&nbsp; <strong>Note:</strong> Rig 4 (single pearl); second cycle added',
...
STRAIN_STATUS = [
    ...
    StrainStatus(name="Blueberry 36 #1",                  profile_anchor="#bb361-profile",    next_text="Try 410°F endpoint on Run 4 — tail harshness consistent at 415°F across Runs 2 and 3", accent=None, slug="bb361",
        info=BB36_1_INFO,
    ...
    StrainStatus(name="Fire Water #106", profile_anchor="#fw106-profile", next_text="Run 17: return to Rig 5, 8s ramp to 420°F, 40s hold — test whether within-draw vapor thinning was Rig 4 single-pearl behavior", accent=None, slug="fw106",
        info=FW106_INFO,
        ...
        next_dab_notes="I confounded the hell out of that run. The part that worked was first cycle first draw, smooth, tasty. The taste to my recollection faded a bit maybe over time in the fridge — makes me wonder about the humidity environment in my fridge where I'm keeping this stuff. There was also a faint crackle and pop — only one crackle in the first draw, never heard again, probably from water left over from the cleaning I had just done to the entire rig. Wispy vapor in that first draw though. Each following draw had the pattern of thicker vapor at first part of draw and then wispy over the length of the draw. This kept going through the rest of the cycle regardless of number of draws. And the harshness was building up. I ran another cycle and waited for it to get to 420 before I drew on it and it was still doing it. Hit it short draws until the harshness warned me off. Swabs were dark golden, surprising amount of reclaim. I was sure I loaded on the normal side. I got as normal a load size as possible with the runny consistency. Pretty stoned. Chatty, creative, some body tension. High intensity.",
        next_ai_analysis="Run 17: back on Rig 5, same 8s ramp to 420°F, 40s hold. The within-draw thick-to-wispy pattern is the open question from Run 16 — if it disappears on Rig 5, single pearl thermal mass is confirmed as the variable. 40s first, not 60s — adding session length would muddy the equipment comparison.",
        next_waypoints=FW106_FASTER_420,
    ),
    StrainStatus(name="Watermellos", profile_anchor="#watermellos-profile", next_text="Run 16: same 60s single cycle at 420°F, larger load — test whether harshness timing tracks load size", accent=None, slug="watermellos",
