...
RUNS = [
    ...
    CompletedRun(
        ...
        analysis='Run 8, same descent curve as Runs 5–7 (440°F open → 420°F @30s → 400°F floor), Rig 6, second dab of the day, normal load, single cycle — ended voluntarily when it felt done. Amber swab with minimal reclaim is the lightest reclaim reading on this curve\'s runs to date, consistent with a normal (not oversized) load being more fully consumed. Coughing and choking read as terp-load cough — dense vapor delivery, documented cross-strain (FW106 R1, Hive #1 R8) as distinct from airway harshness. Mild harshness did appear late in the cycle, chest-located, persisting after the session ended rather than resolving with it. This is notable beyond this jar: Papaya + Z Pie #22 R7, on the same hot-open/gentle-decline curve shape, flagged the same combination — chest location and a post-session hit — as a candidate curve-shape-specific signal rather than a timing or dose question. Two strains, same curve shape, same unusual location and persistence now on record; still early, but a real candidate pattern to watch rather than one-off noise. Medium intensity on normal load continues to sit where expected — between R6\'s light-load medium and R7\'s bigger-load medium-high — supporting load size as the primary intensity lever on this curve, independent of the harshness-location question.\n\n[Equipment corrected to Rig 6 on July 2, 2026 — originally logged Rig 5 via stale jar-local default; user confirmed the Piston has been in continuous use since June 17, 2026. The comparison to Papaya + Z Pie #22 R7 above is therefore same-rig (both Rig 6), not cross-rig.]',
    ),
    CompletedRun(
        strain='Banana Punch #4 + Randy Watzon #13',
        run_date=date(2026, 7, 1),
    ...
    CompletedRun(
        ...
        run_date=date(2026, 7, 2),
        sessions_prior_today=1,
        utc_logged_at=datetime(2026, 7, 3, 2, 13, 28, tzinfo=timezone.utc),
        waypoints=BP4RW13_DESCENT_GENTLE,
        equipment=RIG_6,
        duration_seconds=60,
        endpoint_note='<strong>Open:</strong> 440°F &nbsp;|&nbsp; <strong>Floor:</strong> 400°F — same descent curve as Runs 5–10, second dab of the day',
        swab='amber',
        session_char='Two cycles, two water sips; second cycle depleted after two draws; mild harshness (throat and chest) on second-cycle draws; not overly terpy but tasty throughout; bitter citrus (\'lemon pledge\') on first rip; larger-than-normal load; medium-high intensity',
        intensity='medium-high',
