...
STATUS = StrainStatus(
    ...
    next_text='Run 14 — grey 450°F, modest load, single cycle, first dab: the clean operating point (R9 short draws and R11 long draws both came back clean and functional). Watch whether the chest leg stays absent as insert residue rebuilds between deep cleans — the passive, no-provoke test of the deep-clean/curing coincidence.',
    accent=None,
    slug='sourtangie',
    info=[
    ...
    next_dab_notes='',
    next_ai_analysis='Grey 450°F, modest load, single cycle, first dab is now the jar\'s operating curve: R9 (short draws) and R11 (long draws) both came back clean and functional, so draw length isn\'t a lever to manage here and the modest single-cycle first dab is the reliable read. The live question is the chest leg — absent five straight (R9–R13) since the insert deep-clean and the material\'s curing began together, confounded and possibly just noise (R4 was also chest-free). Cleanest no-provoke test is passive: keep logging and watch whether chest re-emerges as insert residue rebuilds before the next clean — if it returns as the insert dirties, that points at insert state over material cure. Don\'t force it with a deliberate big two-cycle load; that provokes the exact discomfort tracked against the user\'s esophageal anatomy. The throat leg, separately, escalates across a day\'s dabs — first dabs stay cleanest. Expected: clean modest first dab, light-golden/minimal swab, functional medium, throat at most, no chest. Surprising: chest on a clean modest first dab (reopens peak temperature or the anatomy as load-independent), or chest returning specifically as the insert dirties (points at residue).',
