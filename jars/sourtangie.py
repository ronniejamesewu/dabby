...
STATUS = StrainStatus(
    ...
    next_text='Run 11 — grey 450°F, genuinely modest load, first dab, short draws: R9\'s order-matched replication (R10 overshot the load, so the clean modest single-cycle first-dab read still rests on R9 alone). Keep draws short through the dense phase.',
    accent=None,
    slug='sourtangie',
    info=[
    ...
    next_dab_notes='',
    next_ai_analysis='Run 11: grey 450°F is the emerging operating curve — genuinely modest load, first dab, short draws, one clean cycle. Run it as R9\'s order-matched replication; R10 overshot, so R9 is still the lone clean modest first-dab point. Two runs (R9, R10) held the chest/heartburn leg absent whenever draws stayed short through the dense vapor, even at R10\'s bigger load and second cycle — draw technique is the working lever, not load. Don\'t deliberately test the hard-draw-on-full-load cell; that provokes the exact discomfort being tracked (against the user\'s esophageal anatomy). Watch flavor as the material cures — R9/R10 both read slightly less tasty than the fresh-press R3–R5. Expected: one clean 45s cycle, beige-to-gold swab, functional medium, base-of-throat at most, no chest. Surprising: chest/heartburn on a modest short-draw first dab — that reopens peak temperature or the esophageal anatomy as load/technique-independent.',
