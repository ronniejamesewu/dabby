...
RUNS = [
    ...
    CompletedRun(
        ...
        session_char="Ported the grey 450°F bounded-hold + shorter descent from Sour Tangie; load per plan (modest — not restated by user), single cycle depleted by end of the 45s cycle. Ultra-clean beige swab. No harshness and no chest; a big terpene-load punch went up the sinus ('terp shudders'), which the user held distinct from harshness. Second dab of the day. Moderate intensity.",
        intensity="moderate",
        dab_notes="[Pre-dab:] Given all that we have learned about the 450 descent I want to try it on papz. [Load not restated by user — the plan's modest load and single cycle stood uncorrected in the readback.] [Report:] Ultra clean beige swabs, great run, no harshness except some vapor got up my sinus and now I have terp shudders happening. Load was depleted by end of cycle. [Intensity:] I'd say it was moderate. Sarah found her small dab very strong. So it's all relative. You're right on the terps. [Asked at logging: Sarah's dab was not on Dabby tonight — nothing to log for it. No chest after either of tonight's sessions.]",
        analysis="Run 8 ports Sour Tangie's grey 450°F bounded-hold + shorter descent onto this strain, over the plan-on-file 415°F-floor descent — and the deviation is the better run. It answers Run 7's 'maybe could have gone a bit hotter' directly, and the curve that carried eight Sour Tangie runs traveled clean: a single cycle depleted by cycle's end (load per the plan's modest call — the user never restated it). No leftover material on a dead insert this time — the problem Run 7's 400°F floor produced, and the whole reason the 415°F plan existed. The 410°F floor plus the shorter cycle plus a modest load matched depletion without the fix, though the smaller load is confounded with the floor for why it cleared (Run 7 ran larger).\n\nNo harshness, and no chest. Run 7 is the strain's only prior descent and its only chest instance, but it moved load, open, floor, cycle count, and session order all at once against this run — no isolation. What holds: a modest single cycle came back chest-free — the condition that kept Sour Tangie chest-free on this exact curve (Sour Tangie R9/R11, though both were first dabs where this was a 2nd, so order differs) — consistent with the cross-strain read that chest tracks load/cycle/residue, not the hot open itself.\n\nThe 450°F open threw a big terpene-load punch up the sinus — 'terp shudders' — which the user separated from harshness, the distinction Sour Tangie has held since R7. At 450°F terpenes and cannabinoids are all above boiling, so that's the dense peak bolus, not selective delivery; the nasal route and the shudder are new to this strain, single instance.\n\nSwab ultra-clean beige, the lightest this jar has read since Run 4's beige (also Rig 6, single cycle) — within the clean range, within-strain only, not a floor signal.\n\nModerate intensity, down from Run 7's 'very high' and Run 6's 'pretty fucking hard.' The endpoint went up but intensity came down — most parsimoniously the modest load and the short, cleanly-depleted 45s session, not the temperature; second dab of the day is a further order confound.",
    ),
    CompletedRun(
        strain="Papaya + Z Pie #22",
        run_date=date(2026, 7, 16),
        sessions_prior_today=2,
        utc_logged_at=datetime(2026, 7, 17, 4, 9, 11, tzinfo=timezone.utc),
        waypoints=PAPZP22_HOLD_DESCENT_450,
        equipment=RIG_6,
        too_hot=False,
        duration_seconds=45,
        endpoint_note='<strong>Cycle 1:</strong> 450°F open → 410°F floor (grey, 45s) &nbsp;|&nbsp; <strong>Cycle 2:</strong> switched to teal 430°F open → 400°F floor (60s), ended ~14s early',
        swab="beige, with a little dark gold",
        session_char="Two-curve session, 3rd dab of the day, normal load. Cycle 1 on the grey 450°F bounded-hold descent: mild bottom-of-throat harshness after the cycle, heavy terpene-load coughing that cut draws short, big flavor, and 'maybe a hint of toast' in the first draw. Cycle 2 switched to the teal 430°F bounded-hold descent — no reason stated for the switch; 'Really good! That was the answer!' — with no toast mentioned on cycle 2. Terminated with ~14s left: 'seemed wispy' and the user had had enough. Harshness location given as throat; chest not asked at session time, and asked at logging — none during or after either of tonight's sessions. Swab beige with a localized dark-gold spot — per the user, this dab's own material pooled outside the joystick airflow (a technique artifact; the insert is swabbed clean after every dab). Pretty hard.",
        intensity="pretty hard",
        dab_notes="[Pre-dab:] Doing it, and normal load. [Cycle 1, grey 450:] Mild harshness at bottom of throat after first cycle. Tons of terp coughing, a lot of flavor and maybe a hint of toast in the first draw. It occurred to me to try 445 or 440. The coughing caused me to not get a lot of draws, so I'm gonna see about a second cycle. [Switching to the teal 430 curve for second cycle.] [Cycle 2:] Really good! That was the answer! Terminated with 14 seconds left, seemed wispy and I'd had enough. [Swab / insert:] Beige with a little bit of dark gold. I suspect it's from a place where reclaim pooled and didn't get hit with air from the joystick to vaporize. It hit pretty hard. I swab the insert after every dab and I deep clean everything once a week or less. Last time I deep cleaned was Tuesday evening. [Correction at logging, after the analysis overread the spot as insert residue:] pooled reclaim FROM THE DAB THATS BEING CLEANED. it's an airflow issue with the joystick technique rather than leftover reclaim in the insert. the insert is swabbed clean after every dab",
        analysis="Run 9 ran two curves in one session: 3rd dab of the day, normal load, cycle 1 on the grey 450°F bounded-hold descent, then cycle 2 switched to the teal 430°F bounded-hold descent. The user gave no reason for the switch — comfort after the throat harshness and heavy coughing is as plausible as chasing the toast question, and the within-session-A/B framing came from the AI side of the conversation, not the user. The toast observation itself was doubly hedged — 'maybe a hint of toast in the first draw' — and cycle 2 drew no toast mention at all; 'Really good! That was the answer!' is the user's entire cycle-2 read, and it was never explicitly tied to the toast. Read at that weight: one hedged toast note at a 450°F open on fresh material, none mentioned at a 430°F open on the same load's remainder — consistent with the cooler open, and with a 'maybe' that was never there; cycle order and partially-spent material confound the comparison regardless. The user suggested trying 445 or 440 ('it occurred to me') — captured at suggestion weight. Insert state, corrected by the user at logging: the insert is swabbed clean after every dab (deep clean roughly weekly, last Tuesday evening), so prior-dab residue is not a live toast candidate — an earlier draft framed this run's dark-gold swab spot as accumulated residue, and the user corrected it: the spot is this dab's own material pooled where the joystick airflow didn't reach, an airflow/technique artifact, not insert history. That correction supplies the third toast candidate at the same weight: a same-dab pocket sitting on a 450°F insert without airflow degrades in place (the FW106 R31 airflow-gated/heat-gated mechanism at spot scale), and a hotter open cooks it faster. A 450°F rerun reading toast and swab-spot together is the cheap next test.\n\nMild bottom-of-throat harshness after cycle 1 — the within-day mucosal climb Sour Tangie showed across same-day dabs (R11→R13), landing on the 3rd dab where the cumulative-exposure framing expects it. Chest was not asked at session time — the stated location was throat — but the post-session window (where Sour Tangie R14's chest arrived) was checked at logging: the user reported no chest during or after either of tonight's sessions. A genuine no-chest data point on a normal-load 3rd dab, recorded with its retroactive-ask provenance.\n\nHeavy terpene-load coughing off the 450°F open cut the draws short — the dense peak bolus again (Run 8's sinus 'terp shudders'; the terpene-cough this log holds separate from harshness) — likely leaving more material for the second cycle. The session ended with ~14s left in cycle 2 on 'seemed wispy' and having had enough — a satiety stop with a wispy-vapor read consistent with depletion; not a harshness stop, though the mild cycle-1 throat harshness means harshness wasn't absent this session, just not limiting.\n\nSwab beige with the dark-gold spot — mostly the clean range; the spot is, per the user, this dab's own pooled material from the joystick airflow's dead zone — a technique artifact, not a floor or insert-state signal. Two-cycle run, within-strain only. Intensity 'pretty hard,' up from Run 8's moderate — normal load (vs. Run 8's planned-modest, never restated by the user), 3rd-dab order, and two cycles all differ; not isolable.",
    ),
    CompletedRun(
        strain="Papaya + Z Pie #22",
        run_date=date(2026, 7, 22),
        sessions_prior_today=0,
        utc_logged_at=datetime(2026, 7, 23, 2, 7, 57, tzinfo=timezone.utc),
        waypoints=PAPZP22_HOLD_DESCENT_450,
        equipment=RIG_7,
        too_hot=False,
        duration_seconds=45,
        endpoint_note='<strong>Open:</strong> 450°F &nbsp;|&nbsp; <strong>Floor:</strong> 410°F — grey 10s-hold bounded descent, two cycles; Rig 7 (SAML RBR wet top) debut',
        swab="beige, very minimal — ultra clean (rig deep-cleaned just before)",
        session_char="First dab of the day and Rig 7's debut — the SAML RBR (Refined Bell Recycler) wet top replacing the stock bubbler. Grey 450°F bounded-hold descent, two cycles (45s each); large load, depleted a few seconds before the end of cycle 2. Taste definitely muted — the first run of this curve where flavor read down (R8/R9 on the stock bubbler were loud). Beige, very minimal swab, ultra clean, though the rig was deep-cleaned just before, so the clean read is confounded. No toast noted this run (uniform swab, no pooled spot) — a weak point toward R9's 'maybe was noise' branch, with the deep clean confounding the swab. Big intensity. Chest/heartburn onset early — around the 450°F peak (~8s in) — and lingering after; a water sip reduced it only partially. Cycle 2 taken deliberately upright rather than slouched showed next to no added heartburn, raising posture as a candidate lever.",
        intensity='big — "kicked hard"',
        dab_notes="[Cycle 1:] Definitely mutes taste. Heartburn showed up around 37 seconds left. And lingering now. One cycle wasn't enough for the load I guess, I'm gonna run a second one. [Cycle 2 / swab:] Swab is shockingly clean. Beige and very minimal. Oh this is after a deep clean on the rig! That's interesting to note. Effect is feeling pretty big right now but I have been forgetting to close out logs so I want to focus and get this in. I think the heartburn might be related to posture. I'm used to slugging over the stock bubbler but with this new top I don't need to do it. But body memory had me slouched the first cycle. I tried different posture for second cycle and next to no increase in heartburn (which water had brought down somewhat). Which makes me think if I'm conscious of my posture and breathing before every dab, that might be good. Oh the load finally depleted with a few seconds left in the second cycle. I guess I loaded pretty large, which is also a check on the way my dosing has increased — this project has definitely driven my tolerance up. But goddamn if that dab didn't kick hard. I think the slouched position is impeding me inhaling all the vapor into my lungs, some amount is going down my esophagus.",
        analysis="Two firsts on one run: Rig 7's debut — the SAML RBR wet top over the stock bubbler — and the first time this strain's grey 450°F curve read with muted flavor. Taste is the cleanest RBR signal available here: R8 and R9 ran this exact curve on the stock bubbler with loud flavor (the 'terp shudders,' heavy terpene coughing), and R10 held the curve constant while swapping only the top and came back muted. The large load cuts the right way for once — a bigger scoop should throw more flavor, not less — so muted taste on a large load points at the RBR conditioning terpenes out rather than at load or the thumbprint zone lottery. That matches the top's design bet: more vapor cooling and conditioning at some terpene cost. No claim on potency — intensity was big, but the large load explains that on its own, so this run says nothing clean about whether the RBR preserves the punch; that needs matched loads across the two tops.\n\nThe more consequential result is the chest/heartburn. It arrived early — around the 450°F peak, roughly 8s in, the earliest chest onset in this jar (R7 was at the very end) — and lingered, with a water sip only partially reducing it (a partial reduction like OC R15 / LunarZ R3, not a full reset per the water-sip-reset pattern). The RBR cools the vapor harder than the bubbler, and the chest signal came earlier and persisted anyway. That cuts against vapor temperature as the chest driver: if hot vapor on tissue were the mechanism (the thermal-injury-vapor-temp working theory), more cooling should have softened it, not front-loaded it.\n\nWhat the RBR made newly visible is posture. Every prior chest instance — across this jar, Sour Tangie, and LunarZ — happened on the stock bubbler, which forces a slouched, chin-down position; posture was a hidden constant, never a variable. The RBR doesn't require the hunch. Cycle 1 was taken in the old slouch (body memory) and threw the early heartburn; cycle 2, deliberately upright, showed next to no added heartburn. The user's read: slouching keeps some of the draw from fully reaching the lungs, sending vapor toward the esophageal/chest locus already tracked as the candidate site. A mechanical, posture-dependent route fits a heartburn-quality sensation and the temperature-agnostic behavior better than the vapor-heat theories do.\n\nThis is one session and heavily confounded, so the value is direction, not conclusion: cycle 2 changed posture, water, and cycle depletion together, so posture isn't isolated; the ultra-clean beige swab is confounded by the deep clean done just before, so it can't be credited to the top or the curve; and the load was large. The discriminating test is clean because posture should beat both temperature and load — a deliberately upright, breath-conscious grey run on the RBR with no water should come back without the chest signal, and the inverse, a deliberately slouched run at a lower, cooler endpoint, should bring it back. If the chest signal tracks posture instead of the thermometer, a large part of the chest thread reframes as bubbler ergonomics.",
    ),
]
...
STATUS = StrainStatus(
    ...
    next_text='Run 11: rerun the grey 450°F on the RBR (Rig 7), first dab, deliberately upright and breath-conscious from the first draw, no water unless needed, best-achievable modest single cycle — isolate posture as the chest lever (the one variable the bubbler never let vary).',
    accent=None,
    slug='papzp22',
    info=[
