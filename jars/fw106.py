...
RUNS = [
    ...
    CompletedRun(
        ...
        sessions_prior_today=0,
        utc_logged_at=datetime(2026, 6, 24, 23, 30, tzinfo=timezone.utc),
        waypoints=FW106_FASTER_420_60S,
        equipment=RIG_6,
        duration_seconds=60,
    ...
    CompletedRun(
        ...
        sessions_prior_today=0,
        utc_logged_at=datetime(2026, 6, 30, 0, 24, tzinfo=timezone.utc),
        waypoints=FW106_DESCENT_GENTLE,
        equipment=RIG_6,
        duration_seconds=60,
        endpoint_note='<strong>Open:</strong> 440°F &nbsp;|&nbsp; <strong>Floor:</strong> 400°F — gentle descent (440→420@30s→400@60s); larger load; second cycle, one draw',
        swab='Amber, lot of reclaim.',
        session_char='Tasty; dense vapor throughout; draw 1 clean (14s); medium harshness at end of draw 2 (~32s); draw 3 harsh, kept short, water sip; cycle ran to 60s; second cycle, 1 draw, mild harshness post-water.',
        intensity='Very hard, in a good way.',
    ...
    CompletedRun(
        ...
        analysis="First dab of the day, testing the slower-descent/higher-floor idea — as-run it stretched past the plan: 440°F open → 430°F @25s → 420°F @50s, a 50s cycle run twice with a break between. User's own verdict: too gentle — effectively a sustained ~440°F, and now we know what that's like on this rig: near gag-inducing. The comfort limiter was vapor density, not harshness — coughing hard enough for tears buried the mild-to-medium harshness that started ~25s in. That onset zone roughly echoes the old flat-440 run on the spinner (R5, harsh ~28s), but the character is completely different on the joystick: density overwhelms before harshness registers. Two draws on cycle 1 with long pauses; cycle 2 gave one dense coughing hit, then a wispy draw and done.\n\nThe run's apparent puzzle: a larger-than-normal load 'depleted' early yet swabbed out dark amber with heavy reclaim. It dissolves once vaporization is treated as airflow-gated — the material needs vigorous carb-cap draw to come off, and this session was mostly not-drawing (two sparse draws, long coughing pauses, a between-cycle break). A big load plus only a few draws simply leaves most of it in the cup: the heavy reclaim is exactly what's expected, and 'depleted early' was most likely a premature read off a wispy draw, not true exhaustion. The dark amber (darker than the golden/amber of R26–R30) is that material degrading in place under sustained heat — degradation is heat-gated and proceeds whether or not anyone draws, while vaporization does not, so no relocation-to-the-glass and no majority-flashing-early is needed. What the run does show: the efficiency theory it was built to test pointed the wrong way — the descent slope was gentler than the sapphire's ~1°F/s passive decay, so the PID held the insert near 440 (≈ flat 440), and more time at peak cooked material rather than delivering it. One run.\n\nIntensity: very, very intense — ~5 minutes knocked flat with tears, then upbeat, creative, chatty. Consistent with the density story: what did get inhaled came in two very large boluses.\n\n[Mechanism corrected July 7, 2026 (Session 151), by exception: the original analysis attributed the heavy reclaim to material boiling off uninhaled and re-condensing on the glass, and called the load 'relocated.' That is physically wrong — vaporization here is airflow-gated, and the dark amber was insert-side residue degrading in place, not condensate on the glass. The middle paragraph now reflects the airflow-gated / degrade-in-place reading; see the Curve Design Working Theory note in HANDOFF_WISDOM.md.]",
    ),
    CompletedRun(
        strain='Fire Water #106',
        run_date=date(2026, 7, 20),
        sessions_prior_today=0,
        utc_logged_at=datetime(2026, 7, 21, 0, 46, 17, tzinfo=timezone.utc),
        waypoints=FW106_HOLD_DESCENT_450,
        equipment=RIG_6,
        duration_seconds=45,
        endpoint_note='<strong>Open:</strong> 450°F &nbsp;|&nbsp; <strong>Floor:</strong> 410°F — 10s hold at 450°F, then shorter gentle descent (grey); hottest open logged for FW106, large load, two cycles',
        swab='Dark golden — a lot of reclaim; surprisingly light for the hottest open logged.',
        session_char='Dark golden swab with heavy reclaim — surprisingly light for the hottest open FW106 has run (450°F) across a two-cycle session. Large load, first dab of the day, two ~45s cycles with a ~22-min pause between. Cycle 1 wildly terp-forward — mouth- and eye-watering, gag-level terpene-load cough; heartburn set in after the cycle during the coughing. Cycle 2 tasty to the very end, turned toasty only ~3s before depletion. Very hard-hitting.',
        intensity='Very hard.',
