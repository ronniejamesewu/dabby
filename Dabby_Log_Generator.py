...
def rig_reference_html():
    ...
    rows = ""
    for rig, label in _RIG_LABELS:
        insert_cell = _insert_label(rig.insert)

        cap_base, cap_airflow = _cap_label(rig.carb_cap)
        cap_cell = (f"{cap_base}<br><small>{cap_airflow} airflow</small>" if cap_airflow
                    else cap_base)
...
def generate_handoff_state():
    ...
    lines.append(f"*Data current through: {fmt_date(last_date)} (last logged run).*")
    lines.append("")
    lines.append("---")
    lines.append("")
