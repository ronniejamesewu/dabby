...
- **Lifecycle flag on `StrainStatus` — structural endgame of the per-jar refactor.** The per-jar architecture established a rule: per-jar facts live in jar files. Lifecycle state (active vs. closed) is intrinsic to the jar — it describes the physical jar regardless of other jars. Yet lifecycle currently lives in the manifest as list-membership (`ACTIVE` vs. `CLOSED`), for historical reasons: the manifest predates the per-jar split and was the only available home for cross-jar state at the time. The consequence is that closing a jar is a two-file edit — move slug in `jar_manifest.py` AND update prose framing in the jar's `STATUS` — and `_check_closed_tier()` exists purely to guard the gap between these two files. If lifecycle were a field on `StrainStatus` (e.g., `is_closed: bool = False`), `_check_closed_tier()` could become an in-jar `__post_init__` invariant, closing would be a one-file edit, and the manifest would reduce to a pure roster + display-order list — the only information it contains that is genuinely relational. Deferred because it requires changing `Dabby_Core.py` (the schema), all ~17 jar files, the manifest, and the generator simultaneously, with no test suite and `validate()` as the only net. `_check_closed_tier()` is holding the line adequately in the meantime. **Trigger:** a jar-close error that `_check_closed_tier()` misses, or two-file friction causing a real consistency bug.