...
## Last updated: May 14, 2026 — Session 31
...
**Publishing workflow:**
1. Edit `Dabby_Data.py` for new runs and data changes; edit `Dabby_Log_Generator.py` to add the new run's HTML section in `build_html()` and update the strain's `what_to_try_next_html()` call
2. Run `python3 Dabby_Log_Generator.py` to produce `index.html`
3. Commit all three files (`Dabby_Data.py`, `Dabby_Log_Generator.py`, `index.html`) to a feature branch
...
**Adding a new strain requires:** data constants in `Dabby_Data.py` (`*_INFO`, `*_TERPS`, `*_RUN1`, etc.), a `STRAIN_STATUS` entry, a `COMPLETED_RUNS` entry, and section build code in `build_html()`.

**Dataclass constructors (defined in `Dabby_Data.py`):** `COMPLETED_RUNS` uses `CompletedRun(strain=..., run_date=..., sessions_prior_today=..., utc_logged_at=..., waypoints=...)` — `run_date` is a `date` object or `None` if not confirmed; `sessions_prior_today` is an int or `None` if unknown; `utc_logged_at` is a UTC `datetime` (e.g. `datetime(2026, 5, 12, 3, 15, tzinfo=timezone.utc)`) or `None` for pre-dataclass entries. Curve variables are lists of `Waypoint(time_s=..., temp_f=..., note=...)` — both fields are bare `int`, no unit strings. `STRAIN_STATUS` uses `StrainStatus(name=..., profile_anchor=..., next_text=..., accent=..., slug=...)` — `accent` is `None` for auto-assignment or a hex string override; `slug` drives the last-run anchor (`#{slug}-run{n}`). `TERPENE_REFERENCE` uses `TerpeneEntry(name, alias, bp_f, bp_c, band, aroma, qualities, found_in)`. Accent colors are resolved at module load time into `_ACCENT_RESOLVED` dict by distributing hues evenly across the non-green hue space (0–89° and 166–359°). There is no `ACCENT_PALETTE` — do not add one back. `what_to_try_next_html()` helper generates the What to Try Next block; signature: `(section_id, dab_notes, ai_analysis, proposed_waypoints=None, accent=None)` — pass `accent` to apply the strain's color to the section header. `accent_header(title, accent)` returns a colored section header div for use on profile sections. `session_order_note(sessions_prior)` returns a "Nth session of the day — N prior" meta note string when sessions_prior > 0, empty string otherwise. `validate()` runs at build time — catches strain name mismatches between `COMPLETED_RUNS` and `STRAIN_STATUS`, plus waypoint sanity errors. `validate_accent_colors()` checks manually-overridden accent colors only.
...
- Data/code split (Session 31) is done. All session data lives in `Dabby_Data.py`; the generator imports it via `from Dabby_Data import *`. Post-split token counts: data ~9.6K, generator ~17.4K — both files read in a single Claude pass. Do not merge the two files back together.
