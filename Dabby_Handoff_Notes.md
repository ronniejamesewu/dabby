...
## Last updated: May 14, 2026 — Session 30
...
- **Data/code separation** — the next structural step is splitting data definitions (strain info, run curves, COMPLETED_RUNS, STRAIN_STATUS) into a separate file (`Dabby_Data.py`) from generator logic (CSS, helpers, build_html). This solves the context limit problem for the data file (always read in one pass) and makes Claude edits safer — but it does **not** eliminate generator edits for new runs. Each new run still requires adding a `collapsible_section(...)` call and updating `what_to_try_next_html()` in `build_html()`. Eliminating that is a subsequent step (template-driven build), out of scope for this refactor. Prerequisite (dataclasses refactor) is complete as of Session 28. Planning complete as of Session 29; doc reviewed and hardened in Session 30. Key decisions: use `from Dabby_Data import *` in the generator; extract inline MB9ZST proposed waypoints to a named constant `MB9ZST_NEXT` (a discrete new-symbol step, not a copy-paste); `validate()` stays as an explicit call from `build_html()`, not auto-invoked on import; the `# ── SECTIONS ─` divider stays in the generator (not copied to the data file). Generator token count post-split is uncertain — could land 25K–32K tokens; verify after the split. Local planning doc (`REFACTOR_DATA_SPLIT.md`, untracked) has full execution steps, pros/cons, reversibility commands, and preemptive Q&A.
