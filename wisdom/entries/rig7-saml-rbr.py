...
ENTRY = WisdomEntry(
    key='rig7-saml-rbr',
    kind='equipment',
    claim="Rig 7 (Rig 6 + SAML RBR 'Refined Bell Recycler' wet top, replacing the "
          "stock bubbler): the recycler mutes taste and conditions/cools the vapor "
          "harder; it did not reduce the chest/heartburn facet. One run.",
    guidance="Single run. Muted taste is the cleanest signal — expect a "
             "terpene cost. Do not credit the RBR with preserving potency (R10's big "
             "effect rode a large, unisolated load). As the first non-bubbler top it "
             "removes the forced slouch, making posture a variable — see "
             "chest-harshness-hot-open, thermal-injury-vapor-temp.",
    grade='observation',
    grade_basis="Single run (papzp22 R10, July 22, 2026); everything is confounded by "
                "load, a fresh deep clean, posture, and two cycles.",
    evidence=[
        Citation(
            source='papzp22 R10',
            role='confirms',
    ...
    counter_reading="Everything R10 shows is single-run and confounded: muted taste "
                    "could carry a thumbprint load-zone component, the clean swab is a "
                    "deep-clean artifact, and the potency question is unanswerable with "
                    "the load unmatched. Direction only.",
    ...
    updated='papzp22 R10 (July 22, 2026) — RBR debut.',
)
